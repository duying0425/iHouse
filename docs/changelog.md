...
## 2026-07-19 · feat: 语音智能查找助理与 TTS 语音合成 (VoxCPM/浏览器)

**背景**：实现用户通过语音/文本口语化提问物品位置，AI 大模型根据全屋结构化位置索引完成匹配和纠错，并通过语音合成播报给用户。

**功能变更**：

- **智能语音查找助理 (`POST /api/ai/assistant`)**：
  - 新增后端 `server/ai-assistant.js` 整合全屋物品位置信息，构建专有 Prompt 调用 LLM 大模型。
  - 支持 **推理模型自动兼容与过滤**：智能解析大模型返回的结果，过滤类似 DeepSeek R1 产生的 `<think>...</think>` 思考过程标记；当模型未输出标准的 JSON 时，后端会自动触发纯文本降级兜底逻辑，直接返回提取的自然语言文本（避免崩溃）。
- **TTS 语音代理服务 (`POST /api/ai/tts`)**：
  - 新增后端 `server/ai-tts.js`，支持通过 `.env` 中的 `TTS_API_URL` 代理接入 **VoxCPM** 或其他兼容 OpenAI 语音合成规范的服务，提供高保真的自然人声或定制克隆音色。
  - 如果未配置外部 TTS API，后端向前端返回 `TTS_NOT_CONFIGURED`，前端自动平滑降级使用浏览器自带的本地 `window.speechSynthesis` 朗读。
- **前端悬浮语音助手 (`VoiceAssistant.tsx`)**：
  - 在前端主页右下角挂载了一个常驻的 `Sparkles` 魔法悬浮按钮；同时在搜索页面输入框右侧集成了麦克风按钮。
  - 接入浏览器原生 `webkitSpeechRecognition` API 进行高灵敏的语音输入。
  - 使用自适应抽屉布局（移动端全屏 `w-full h-full`、PC 端右侧挂载 `max-w-md` 侧边栏）。
  - 支持将匹配的物品直接以 `ItemCard` 渲染在气泡对话下方，点击即可一键导航直达档案。

**验证**：
- 新增 `server/ai-assistant.test.js`，设计 7 个单元测试用例，覆盖了大模型 Prompt 组装、推理思考链过滤、纯文本兜底、TTS 环境变量及代理转发等，测试 100% 通过；
- 打包构建 `pnpm build` 及 Vitest 完整 280 个测试用例回归全部绿灯。
